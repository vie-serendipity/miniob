    ...
    SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  unique_ptr<LogicalOperator> project_oper(new ProjectLogicalOperator(all_fields, select_stmt->aggregations()));
  if (predicate_oper) {
    if (table_oper) {
      predicate_oper->add_child(std::move(table_oper));
    }
  ...
  UpdateStmt *update_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  vector<Field> fields(update_stmt->fields());
  ...
  unique_ptr<LogicalOperator> table_get_oper(new TableGetLogicalOperator(table, all_fields, false/*readonly*/));
