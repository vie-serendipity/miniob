...
RC ExecuteStage::handle_request_with_physical_operator(SQLStageEvent *sql_event)
{
  ...
  switch (stmt->type()) {
    case StmtType::SELECT: {
      ...
      if (select_stmt->aggregations().size() > 0) {
        for (const Aggregation &agg : select_stmt->aggregations()) {
          string field_name = agg_fun_to_string(agg.agg_fun);
          field_name += "(" + agg.attribute_name + ")";
