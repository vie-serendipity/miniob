...
RC BplusTreeHandler::create(const char *file_name, std::vector<AttrType> &attr_types, std::vector<int> &attr_lengths, int internal_max_size /* = -1*/,
    int leaf_max_size /* = -1 */)
{
  ...
  int attr_sum = 0;
  for (int element : attr_lengths){
    attr_sum += element;
  }

  if (internal_max_size < 0) {
    internal_max_size = calc_internal_page_capacity(attr_sum);
  }
  if (leaf_max_size < 0) {
    leaf_max_size = calc_leaf_page_capacity(attr_sum);
  }

  char *pdata = header_frame->data();
  IndexFileHeader *file_header = (IndexFileHeader *)pdata;
  file_header->attr_lengths = attr_lengths;
  file_header->key_length = attr_sum + sizeof(RID);
  file_header->attr_types = attr_types;
  file_header->attr_sum = attr_sum;
...
MemPoolItem::unique_ptr BplusTreeHandler::make_key(const char *user_key, const RID &rid)
{
  ...
  memcpy(static_cast<char *>(key.get()), user_key, file_header_.attr_sum);
  memcpy(static_cast<char *>(key.get()) + file_header_.attr_sum, &rid, sizeof(rid));
  return key;
}
...
RC BplusTreeHandler::delete_entry(const char *user_key, const RID *rid)
{
  ...
  memcpy(key, user_key, file_header_.attr_sum);
  memcpy(key + file_header_.attr_sum, rid, sizeof(*rid));
    ...
    const char *user_key, int key_len, bool want_greater, char **fixed_key, bool *should_inclusive)
{
  ...
  *should_inclusive = false;
  ...
  int32_t attr_length = tree_handler_.file_header_.attr_sum;
